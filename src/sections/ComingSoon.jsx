...
import { TbX, TbLayoutList } from 'react-icons/tb';
...
function ComingSoon({classes}) {
  ...
  return (
    <section className={`${classes} relative flex flex-col p-4 items-center justify-center rounded-2xl group preserve-3d`}>
      <img src="./img/bg_section_services.jpg" className="m-4 group-hover:translate-z-[10px] object-cover h-40 w-full rounded-[20px]" />
      <button onClick={handleOpenModal} className="text-4xl group-hover:scale-125">Servicios</button>
      <ReactModal
        isOpen={isOpen}
